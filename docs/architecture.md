...
endpoints are in `.env.json`; a single config change swaps the model. The
Anthropic key is read from the nested `anthropic_api_keys.rent-checker` path
in `.env.json`, matching the per-agent key convention used across the shared
`.env.json` on this host.
