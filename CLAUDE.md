...
- Subject line prefix is configurable via email_subject_prefix in
  agent_config.json (e.g., "[Agent - Rent Check]"). Full subject:
  "{prefix} {date} - All Received" or
  "{prefix} {date} - ACTION NEEDED"
...
- Intercepts the initial page load to capture the auth token and
  GraphQL query structure from Web_GetTransactionsList
- Then makes a direct HTTP request to the GraphQL API with
  limit=100 (instead of the UI's default 25) to get all
  transactions in one call
- Scroll-based pagination does not work — the virtualized list
  does not trigger new API calls on scroll
