...
def send_notification(
    ...
    *,
    ...
    """
    subject = _build_subject(results, run_date, error=error_message is not None)
...
def send_error_notification(
    ...
    *,
    ...
    """
    subject = f"[Rent Check] {run_date} - PIPELINE ERROR"
    body = (
        f"The rent payment checker failed on {run_date}.\n\n"
        f"Error:\n{error_message}\n\n"
        "No payment results are available. The run was not logged as complete.\n"
        "Please investigate and re-run manually."
    )
...
def _build_subject(
    ...
    *,
    ...
    if error:
        return f"[Rent Check] {run_date} - PIPELINE ERROR"
    ...
    return f"[Rent Check] {run_date} - {suffix}"
...
def _generate_body(
    ...
    try:
        body = _call_ollama_for_summary(config.ollama_endpoint, config.ollama_model, prompt)
        if not body.strip():
            raise ValueError("Ollama returned an empty response")
        logger.info("Email body generated by LLM")
        if error_message:
            body = f"NOTE: The run completed but an error occurred:\n{error_message}\n\n{body}"
        return body, True
    except Exception as exc:
...
def _fallback_body(
    ...
    *,
    ...
    """Produce a plain-text email body when LLM is unavailable."""
    lines: list[str] = []

    if llm_unavailable:
        lines.append(
            "NOTE: LLM review and email generation were unavailable — "
            "showing raw results only.\n"
        )

    if error_message:
        lines.append(f"NOTE: An error occurred during this run:\n{error_message}\n")
    ...
    if needs_attention:
        ...
        lines.append(
            f"ACTION NEEDED: {len(attention_props)} of {len(results)} properties "
            f"require attention: {', '.join(attention_props)}\n"
        )
    else:
        lines.append(f"All {len(results)} rent payments received.\n")

    lines.append(f"Rent check for: {run_date.strftime('%B %Y')}\n")
    lines.append("-" * 50)

    for r in results:
        label = STATUS_LABELS.get(r.status, r.status.value)
        lines.append(f"\n{r.property_name}: {label}")

        if r.matched_transaction:
            t = r.matched_transaction
            lines.append(
                f"  Transaction: {t['description']}  |  "
                f"${t['amount']:.2f}  |  {t['date']}  |  {t['account']}"
            )
            if t["category"]:
        ...
        lines.append(f"  Resolved by: Step {r.step_resolved_by or '—'}")

    lines.append("\n" + "-" * 50)
    lines.append("Generated by rent-payment-checker-agent.")
